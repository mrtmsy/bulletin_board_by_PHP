<?php
	...
	if(!empty($_POST)){
		$statement = $pdo->prepare('INSERT INTO members SET name=?, email=?, password=?, picture=?, created=NOW(), modified=NOW()');
		$statement->bindvalue(1,"%{$name}%",PDO::PARAM_STR);
		$statement->bindvalue(2,"%{$email}%",PDO::PARAM_STR);
		$statement->bindvalue(3,"%{$password}%",PDO::PARAM_STR);
		$statement->bindvalue(4,"%{$picture}%",PDO::PARAM_STR);
		$statement->bindvalue(5,"%{$created}%",PDO::PARAM_STR);
